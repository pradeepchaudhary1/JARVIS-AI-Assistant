...
try:
    ...
    from memory.long_memory import (
        handle_long_memory_command, build_full_memory_context,
        load_user_profile, load_project_memory
    )
    LONG_MEMORY_OK = True
except Exception:
...
def get_ai_reply(user_text, use_backup=False):
    ...
    if LONG_MEMORY_OK:
        system_prompt += build_full_memory_context()
    elif MEMORY_OK:
...
def handle_web_command(text):
    ...
    if PERSONALITY_OK:
        ...
        if personality_result:
            return personality_result, "personality"
    ...
    if any(w in t for w in ["status", "system check", "kya chal raha"]):
        ...
        if LONG_MEMORY_OK: parts.append("LongMemory ON")
        if PERSONALITY_OK: parts.append("Personality ON")
...
if FLASK_OK:
    ...
    def health():
        return jsonify({
            ...
            "memory": MEMORY_OK, "long_memory": LONG_MEMORY_OK, "worldmonitor": WORLDMONITOR_OK,
    ...
    def get_profile():
        if not LONG_MEMORY_OK:
            return jsonify({"error": "long_memory module not loaded"}), 500
    ...
    def get_projects():
        if not LONG_MEMORY_OK:
            return jsonify({"error": "long_memory module not loaded"}), 500
        return jsonify(load_project_memory())

    @app.route("/personality", methods=["GET"])
    def get_personality():
        if not PERSONALITY_OK:
            return jsonify({"error": "personality module not loaded"}), 500
        return jsonify(load_personality())

    @app.route("/personality", methods=["POST"])
    def set_personality_field():
        if not PERSONALITY_OK:
            return jsonify({"error": "personality module not loaded"}), 500
        ...
        from soul.personality_engine import update_trait
        msg = update_trait(key, value)
        return jsonify({"message": msg, "personality": load_personality()})
