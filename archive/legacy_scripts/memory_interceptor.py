...
async def inject_memory_context(user_text: str, system_prompt: str = "") -> tuple:
    """
    ...
    try:
        # ✅ FIX: get_recent_conversations is NOT async — await हटाया
        memory_context = get_recent_conversations_text(limit=10)
...
async def process_with_memory(user_input: str, base_instructions: str = "") -> dict:
    ...
    Returns dict with keys: has_memory_request, context, enhanced_instructions
    """
    ...
    try:
        # ✅ FIX: await हटाया — sync function है
        context = get_recent_conversations_text(limit=10)
