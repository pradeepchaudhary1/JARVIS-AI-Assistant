...
def _detect_format(user_message: str, context: dict | None) -> str:
    ...
    if ".json" in lowered or "json" in lowered:
        return "json"
    if ".docx" in lowered or "docx" in lowered or "word file" in lowered or "word document" in lowered:
        return "docx"
...
def _write_file(file_type: str, filename: str, payload: Any) -> dict[str, Any]:
    ...
    if file_type == "docx":
        ...
        if isinstance(payload, str):
            paragraphs = payload.splitlines() or [payload]
        ...
        return {
            ...
            "message": f"DOCX file created at {output_path}",
        }
...
def execute(user_message: str, context: dict | None) -> dict[str, Any]:
    ...
    try:
        ...
        if context.get("session_context"):
            context["session_context"].set_last_artifact(
                result["path"],
                file_type,
            )
